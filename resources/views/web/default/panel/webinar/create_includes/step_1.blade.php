...
<div class="row">
    <div class="col-12 col-md-4 mt-15">
        ...
        <div class="form-group mt-15">
            <input type="checkbox" {{ (empty($webinar) || (!empty($webinar) && $webinar->visible_to_all == true )) ? 'checked' : '' }} name="visible_to_all" id="visible_to_all" >
            <label for="visible_to_all" class="input-label"> {{ trans("public.visible_to_all") }} </label>
        </div>

        <div class="form-group mt-15">
            <label class="input-label"> {{ trans("public.visible_to_organizations") }} </label>
            <select class="form-control organizations select2 @error('organizations') is-invalid @enderror" {{ (empty($webinar) || (!empty($webinar) && $webinar->visible_to_all == true )) ? "disabled" : "" }} id="organizations" name="organizations[]"
            data-url="{{ route('admin.get.organization.sites') }}" multiple>
                @foreach ($organization as $org)
                <option value="{{$org->id}}" {{ ( !empty($webinar) && in_array($org->id, $courseVisibility)) ? 'selected' :''}}>{{$org->full_name}}</option>
                @endforeach
            </select>
            @error('organizations')
            <div class="invalid-feedback">
