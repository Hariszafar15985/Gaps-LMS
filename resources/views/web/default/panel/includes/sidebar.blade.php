@php
$sidebar = App\Models\SettingSidebar::getSidebarSettings();
@endphp
...
<div class="panel-sidebar pt-50 pb-25 px-25" id="panelSidebar">
    ...
    <ul class="sidebar-menu pt-10 @if(!empty($authUser->userGroup)) has-user-group @endif" data-simplebar @if((!empty($isRtl) and $isRtl)) data-simplebar-direction="rtl" @endif>
            ...
            <li class="sidenav-item {{ (request()->is('panel/students') or request()->is('panel/manage/students*')) ? 'sidenav-item-active' : '' }}">
                ...
                <div class="collapse {{ (request()->is('panel/students') or request()->is('panel/manage/students*')) ? 'show' : '' }}" id="studentsCollapse">
                    <ul class="sidenav-item-collapse">
                        ...
                        <li class="mt-5 {{ (request()->is('panel/manage/students/behindSchedule')) ? 'active' : '' }}">
                            <a href="{{route('panel.students.behindSchedule')}}">{{ trans('panel.behind_schedule') }}</a>
                        </li>
                    </ul>
                </div>
            </li>
        ...
        @if ($authUser->isOrganization())
            <li class="sidenav-item {{ (request()->is('panel/organizationSites') or request()->is('panel/organizationSites*')) ? 'sidenav-item-active' : '' }}">
                <a class="d-flex align-items-center" data-toggle="collapse" href="#organizationSiteCollapse" role="button" aria-expanded="false" aria-controls="organizationSiteCollapse">
                <span class="sidenav-item-icon mr-10">
                    @include('web.default.panel.includes.sidebar_icons.teachers')
                </span>
                    <span class="font-14 text-dark-blue font-weight-500">{{ trans('public.organization_sites') }}</span>
                </a>

                <div class="collapse {{ (request()->is('panel/organizationSites') or request()->is('panel/manage/organizationSites*')) ? 'show' : '' }}" id="organizationSiteCollapse">
                    <ul class="sidenav-item-collapse">
                        <li class="mt-5 {{ (request()->is('panel/organizationSites/new')) ? 'active' : '' }}">
                            <a href="{{ route('panel.get.new.organizationSite') }}">{{ trans('public.new') }}</a>
                        </li>
                        <li class="mt-5 {{ (request()->is('panel/manage/organizationSites')) ? 'active' : '' }}">
                            <a href="{{ route('panel.manage.organizationSites') }}">{{ trans('public.list') }}</a>
                        </li>
                    </ul>
                </div>
            </li>
            <li class="sidenav-item {{ (request()->is('panel/manage/managers') or request()->is('panel/manage/managers*')) ? 'sidenav-item-active' : '' }}">
                <a class="d-flex align-items-center" data-toggle="collapse" href="#organizationManagerCollapse" role="button" aria-expanded="false" aria-controls="organizationManagerCollapse">
                <span class="sidenav-item-icon mr-10">
                    <img src="{{asset('assets/default/img/activity/48.svg')}}" style="width:28px;border-radius: 50%;" />
                </span>
                    <span class="font-14 text-dark-blue font-weight-500">{{ trans('public.organization_managers') }}</span>
                </a>

                <div class="collapse {{ (request()->is('panel/manage/managers') or request()->is('panel/manage/managers/*')) ? 'show' : '' }}" id="organizationManagerCollapse">
                    <ul class="sidenav-item-collapse">
                        <li class="mt-5 {{ (request()->is('panel/manage/managers/new')) ? 'active' : '' }}">
                            <a href="{{ route('panel.manage.create.user', ['user_type' => 'managers']) }}">{{ trans('public.new') }}</a>
                        </li>
                        <li class="mt-5 {{ (request()->is('panel/manage/managers')) ? 'active' : '' }}">
                            <a href="{{ route('panel.manage.list.users', ['user_type' => 'managers']) }}">{{ trans('public.list') }}</a>
                        </li>
                    </ul>
                </div>
            </li>
        ...
        @if ($authUser->isOrganization() || $authUser->isOrganizationManager())
            <li class="sidenav-item {{ (request()->is('panel/manage/sub_managers') or request()->is('panel/manage/sub_managers*')) ? 'sidenav-item-active' : '' }}">
                <a class="d-flex align-items-center" data-toggle="collapse" href="#organizationSubManagerCollapse" role="button" aria-expanded="false" aria-controls="organizationSubManagerCollapse">
                <span class="sidenav-item-icon mr-10">
                    <img src="{{asset('assets/default/img/icons/skills.svg')}}" />
                </span>
                    <span class="font-14 text-dark-blue font-weight-500">{{ trans('public.organization_sub_managers') }}</span>
                </a>

                <div class="collapse {{ (request()->is('panel/manage/sub_managers') or request()->is('panel/manage/sub_managers/*')) ? 'show' : '' }}" id="organizationSubManagerCollapse">
                    <ul class="sidenav-item-collapse">
                        <li class="mt-5 {{ (request()->is('panel/manage/sub_managers/new')) ? 'active' : '' }}">
                            <a href="{{ route('panel.manage.create.user', ['user_type' => 'sub_managers']) }}">{{ trans('public.new') }}</a>
                        </li>
                        <li class="mt-5 {{ (request()->is('panel/manage/sub_managers')) ? 'active' : '' }}">
                            <a href="{{ route('panel.manage.list.users', ['user_type' => 'sub_managers']) }}">{{ trans('public.list') }}</a>
                        </li>
                    </ul>
                </div>
            </li>
            {{-- Consultants --}}
            <li class="sidenav-item {{ (request()->is('panel/manage/consultants') or request()->is('panel/manage/consultants*')) ? 'sidenav-item-active' : '' }}">
                <a class="d-flex align-items-center" data-toggle="collapse" href="#organizationConsultantCollapse" role="button" aria-expanded="false" aria-controls="organizationConsultantCollapse">
                <span class="sidenav-item-icon mr-10">
                    <img src="{{asset('assets/default/img/activity/60.svg')}}" style="width:28px"/>
                </span>
                    <span class="font-14 text-dark-blue font-weight-500">{{ trans('public.organization_consultants') }}</span>
                </a>

                <div class="collapse {{ (request()->is('panel/manage/consultants') or request()->is('panel/manage/consultants/*')) ? 'show' : '' }}" id="organizationConsultantCollapse">
                    <ul class="sidenav-item-collapse">
                        <li class="mt-5 {{ (request()->is('panel/manage/consultants/new')) ? 'active' : '' }}">
                            <a href="{{ route('panel.manage.create.user', ['user_type' => 'consultants']) }}">{{ trans('public.new') }}</a>
                        </li>
                        <li class="mt-5 {{ (request()->is('panel/manage/consultants')) ? 'active' : '' }}">
                            <a href="{{ route('panel.manage.list.users', ['user_type' => 'consultants']) }}">{{ trans('public.list') }}</a>
                        </li>
                    </ul>
                </div>
            </li>
        ...
        <li class="sidenav-item {{ (request()->is('panel/webinars') or request()->is('panel/webinars/*')) ? 'sidenav-item-active' : '' }}">
            ...
            <div class="collapse {{ (request()->is('panel/webinars') or request()->is('panel/webinars/*')) ? 'show' : '' }}" id="webinarCollapse">
                <ul class="sidenav-item-collapse">
                    ...
                    @if((!empty($authUser->organ_id) && !$authUser->isUser()))
                    ...
                    <li class="mt-5 {{ (request()->is('panel/webinars/purchases')) ? 'active' : '' }}">
                        <a href="/panel/webinars/courses">My Courses</a>
                    </li>
                    @endif
        ...
        <li class="sidenav-item {{ (request()->is('panel/meetings') or request()->is('panel/meetings/*')) ? 'sidenav-item-active' : '' }}">
            ...
            <div class="collapse {{ (request()->is('panel/meetings') or request()->is('panel/meetings/*')) ? 'show' : '' }}" id="meetingCollapse">
                <ul class="sidenav-item-collapse">
                    ...
                    @endif --}}
                    <li class="mt-5 {{ (request()->is('panel/meetings/newMeeting')) ? 'active' : '' }}">
                        <a href="/panel/meetings/newMeeting">{{ trans('public.book_a_meeting') }}</a>
                    </li>

                    <li class="mt-5 {{ (request()->is('panel/meetings/getOrganizationMeetings')) ? 'active' : '' }}">
                        <a href="/panel/meetings/getOrganizationMeetings">{{ trans('public.list') }}</a>
                    </li>
                </ul>
            </div>
            @endif
            ...
            <div class="collapse {{ (request()->is('panel/quizzes') or request()->is('panel/quizzes/*')) ? 'show' : '' }}" id="quizzesCollapse">
                <ul class="sidenav-item-collapse">
                    @if($authUser->isOrganization() || $authUser->isTeacher())
                        ...
                        <li class="mt-5 {{ (request()->is('panel/quizzes/pending')) ? 'active' : '' }}">
                            <a href="/panel/quizzes/pending">{{ trans('quiz.pending_assessments') }}</a>
                        </li>
        ...
        @if(!$authUser->isTeacher() && !$authUser->isUser())
                ...
                <div class="collapse {{ (request()->is('panel/financial') or request()->is('panel/financial/*')) ? 'show' : '' }}" id="financialCollapse">
                    <ul class="sidenav-item-collapse">
                        ...
                        <li class="mt-5 {{ (request()->is('panel/financial/summary')) ? 'active' : '' }}">
                            @if ($authUser->isOrganization())
                                <a href="/panel/financial/organizationSummary">{{ trans('financial.financial_summary') }}</a>
                            @else
        ...
        @if(!$authUser->isTeacher() && !$authUser->isUser() && !$authUser->isOrganization())
        ...
        @if( $authUser->isAdmin() or $authUser->isOrganization() )
            <li class="sidenav-item {{ (request()->is('panel/notifications/students/settings')) ? 'sidenav-item-active' : '' }}">
                <a href="/panel/notifications/students/settings" class="d-flex align-items-center">
                <span class="sidenav-notification-icon sidenav-item-icon mr-10">
                        @include('web.default.panel.includes.sidebar_icons.notifications')
                    </span>
                    <span class="font-14 text-dark-blue font-weight-500">{{ trans('panel.student_notifications_settings') }}</span>
                </a>
            </li>
        @endif
    ...
    @if(!empty($getPanelSidebarSettings))
        ...
        <div class="sidebar-create-class d-none">
