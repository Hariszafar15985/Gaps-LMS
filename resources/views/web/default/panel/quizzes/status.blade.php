...
<section class="container text-course-content-section mt-10 mt-md-40">
    <div class="row">
        <div class="col-12 col-md-8 pt-4">
            <div class="container">
                @if($quizResult->status != \App\Models\QuizzesResult::$failed)
                <section class="mt-40">
                ...
                <section class="mt-30 rounded-lg shadow-sm py-25 px-20">
                        ...
                        @case(\App\Models\QuizzesResult::$failed)
                            <div class="no-result status-failed mt-50 d-flex align-items-center justify-content-center flex-column">
                                ...
                                <div class="d-flex align-items-center flex-column mt-30 text-center">
                                    <h2 class="section-title">{{ trans('quiz.status_failed_thanks') }}</h2>
                                    ...
                                    <div class=" mt-25">
                                        ...
                                        @endif --}}
                                        <a href="{{ route("webinars", ["slug" => $slug]) }}" class="btn btn-sm btn-primary">{{ trans('public.back_to_course') }}</a>
                                        {{-- <a href="/panel/quizzes/my-results" class="btn btn-sm btn-primary">{{ trans('public.back_to_course') }}</a> --}}
                                    </div>
                                </div>
                            </div>
                        @break
                ...
                @if($quizResult->status != \App\Models\QuizzesResult::$failed)
                <x-course.lesson-navigation-buttons :webinar_id={{$quiz->webinar->id}} :previous=$previous :next=$next :showFullScreenButton=false />
                @endif
            </div>
        </div>
