<div id="matchingListTextQuestionModal" class="@if(!empty($quiz)) matchingListTextQuestionModal{{ $quiz->id }} @endif {{ empty($question_edit) ? 'd-none' : ''}}
    ...
    <div class="custom-modal-body">
        ...
        <div class="quiz-questions-form" data-action="/panel/quizzes-questions/{{ empty($question_edit) ? 'store' : $question_edit->id.'/update' }}" >
            ...
            <input type="hidden" name="ajax[type]" value="{{ \App\Models\QuizzesQuestion::$matchingListText }}">
            <div class="row mt-3">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="form-group">
                        ...
                        <input name="ajax[title]" id="" class="form-control" value="{{ !empty($question_edit) ? $question_edit->title : '' }}" />
                    </div>
                </div>
            </div>
            <hr class="py-3" />
                ...
                <div class="col-12">
                    ...
                    <div class="row">
                        <div class="col-3 ml-auto">
                            <button type="button" id="add_text_pair" @if(!empty($question_edit->id) && $question_edit->id > 0) onclick="addPair('question_{{$question_edit->id}}', 'text');" @endif class="add_pair btn btn-sm btn-primary">{{ trans('quiz.add_another_pair') }}</button>
                        </div>
                    </div>
                            ...
                            @if(isset($answers) && count($answers) > 0)
                                ...
                                @foreach($answers as $key => $answer)
                                    <div class="col-3 {{(isset($key) && $key > 0) ? 'pair_container' : ''}}">
                                        <div id="pair_{{$key}}" class="row matchingPair {{($key === 0) ? "permanentPair" : ""}}">
                                            <div class="col-12">
                                                @if($key > 0)
                                                <button type="button" class="removePair-btn btn btn-sm btn-background-transparent text-danger position-absolute" 
                                                title="Remove Pair" onclick="removePair({{$key}}, 'question_{{$question_edit->id}}');" 
                                                style="right:0; height:1em; font-size:1.5em"><span aria-hidden="true">&times;</span></button>
                                                @endif
                                                {{-- Text --}}
                                                <div class="form-group">
                                                    <label class="input-label">{{ trans('quiz.text') }}</label>
                                                    <input type="text" name="answers[{{$key}}][text]" class="form-control" required value="{{!empty($answer['text']) ? $answer['text'] : ""}}" />
                                                </div>
                                                {{-- Description --}}
                                                <div class="form-group">
                        ...
                        <div class="col-3">
                            <div id="pair_0" class="row matchingPair permanentPair">
                                <div class="col-12">
                                    ...
                                    <div class="form-group">
                                        <label class="input-label">{{ trans('quiz.text') }}</label>
                                        <input type="text" name="answers[0][text]" class="form-control" required value="" />
                                    </div>
                                    {{-- Description --}}
                                    <div class="form-group">
                                        <label class="input-label">{{ trans('quiz.description') }}</label>
