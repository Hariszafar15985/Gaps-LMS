<div id="matchingListImageQuestionModal"
    class="@if (!empty($quiz)) matchingListImageQuestionModal{{ $quiz->id }} @endif {{ empty($question_edit) ? 'd-none' : '' }}
    @if (!empty($question_edit->id) && $question_edit->id > 0) question_{{ $question_edit->id }} @endif
    ">
    <div class="custom-modal-body matchingListImageQuestionModal_modal">
        <h2 class="section-title after-line">{{ trans('quiz.matchingListImage_question') }}</h2>
        ...
        <div class="quiz-questions-form"
            ...
            <input type="hidden" name="ajax[type]" value="{{ \App\Models\QuizzesQuestion::$matchingListImage }}">
            <div class="row mt-3">
                ...
                <div class="col-12 col-md-4 ml-auto">
                    <div class="form-group">
                        ...
                        <input type="text" name="ajax[grade]" class="form-control"
            ...
            <div class="row">
                <div class="col-12">
                    <div class="form-group">
                        ...
                        <input name="ajax[title]" id="" class="form-control"
                ...
                <div class="col-12">
                    ...
                    <div class="row">
                        <div class="col-3 ml-auto">
                            <button type="button" id="add_image_pair"
                                @if (!empty($question_edit->id) && $question_edit->id > 0) onclick="addPair('question_{{ $question_edit->id }}', 'image');" @endif
                                ...
                                <div class="col-3 {{ isset($key) && $key > 0 ? 'pair_container' : '' }}">
                                    <div id="pair_{{ $key }}"
                                        ...
                                        <div class="col-12">
                                            ...
                                            {{-- Image --}}
                                            <div class="form-group">
                                                <label class="input-label">{{ trans('quiz.image') }}</label>
                                                <div class="dropzone" onclick="inputFileClicked(this)">
                                                    <img src="{{ isset($answer['image']) ? "{$answer['image']}" : '/assets/default/img/upload.svg' }}"
                                                        id="preview_{{ $key }}"
                                                        class="{{ isset($answer['image']) || !empty($answer['image']) ? '' : 'upload-icon' }}"
                                                        width="150" height="350">

                                                    <input type="file" accept="image/*"
                                                        name="answers[{{ $key }}][file]"
                                                        id="file_{{ $key }}"
                                                        onchange="handleFileSelect(this, this.closest('.matchingListImageQuestionModal_modal'))"
                                                        required class="upload-input d-none">

                                                    <input type="hidden"
                                                        name="answers[{{ $key }}][image]"
                                                        id="image_{{ $key }}"
                                                        value="{{ isset($answer['image']) ? "{$answer['image']}" : '' }}" />
                                                </div>

                                                {{-- <textarea type="text" name="answers[{{$key}}][description]" class="summernote form-control" required >{{!empty($answer['description']) ? $answer['description'] : ""}}</textarea> --}}
                                            </div>
                                            {{-- Text --}}
                                            <div class="form-group">
                        ...
                        <div class="col-3">
                            <div id="pair_0" class="row matchingPair permanentPair">
                                <div class="col-12">
                                    ...
                                    <div class="form-group">
                                        <label class="input-label">{{ trans('quiz.image') }}</label>
                                        <div class="dropzone mx-auto cursor-pointer" onclick="inputFileClicked(this)">
                                            <img src="/assets/default/img/upload.svg" id="preview_01"
                                                class="upload-icon" width="150" height="350">
                                            <input type="file" name="answers[0][file]" id="file_01"
                                                onchange="handleFileSelect(this, this.closest('.matchingListImageQuestionModal_modal') )"
                                                accept="image/*" required class="d-none upload-input cursor-pointer" />
                                            <input type="hidden" name="answers[0][image]" id="image_01"
                                                value=""/>
            ...
            <input type="hidden" name='pairIdentifier'
                value="{{ isset($count) && $count > 0 ? $count - 1 : 0 }}" />
