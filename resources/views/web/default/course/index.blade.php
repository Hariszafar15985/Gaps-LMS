        ...
        <div class="row">
                ...
                <div class="course-content-body user-select-none">
                    <div class="course-body-on-cover text-white">
                        ...
                        @if($hasBought or $course->isWebinar())
                            @php
                                ...
                                $requestedUser = (session('requestedUser')) ? session()->get('requestedUser') : '';

                                } else {
                                    $requestedUser = auth()->user();
                                }
                                $percent = $course->getProgress(($requestedUser != "") ? $requestedUser->id : -1);
                                $expectedProgress = $course->getExpectedProgress(($requestedUser != "") ? $requestedUser->id : -1);
                            ...
                            <div class="mt-20 d-flex align-items-center">
                                <div class="progress course-progress flex-grow-1 shadow-xs rounded-sm">
                                    <span class="progress-bar rounded-sm {{($percent >= $expectedProgress) ? "bg-primary" : "bg-warning"  }}" style="width: {{ $percent }}%"><span  class="popOver" data-toggle="tooltip" data-placement="top" title="85%"> </span></span>
                                    @if (isset($expectedProgressPercent) && $expectedProgressPercent > 0)
                                        <span class="progress-bar rounded-right-sm bg-danger" style="width: {{ $expectedProgressPercent }}%"><span  class="popOver" data-toggle="tooltip" data-placement="top" title="85%"> </span></span>
                                    @endif
                                </div>
                    ...
                    <div class="@if(!$course->isCourse()) mt-35 @else mt-20 pt-20  mt-md-40 pt-md-40  @endif">
                        <ul class="nav nav-tabs bg-secondary rounded-sm p-15 d-flex align-items-center justify-content-between" id="tabs-tab" role="tablist">
                            ...
                            @if( env("SHOW_COURSE_REVIEWS", false) == true)
                            <li class="nav-item">
            ...
            <div class="course-content-sidebar col-12 col-lg-4 mt-25 mt-lg-0">
                <div class="rounded-lg shadow-sm">
                    ...
                    @if(
                        !auth()->user()->isUser()
                        || (auth()->user()->isUser() && env("SHOW_PRICE_VIEW_FOR_STUDENTS", false) == true)
                    )
                    <div class="px-20 pb-30">
                        ...
                        {{-- Section on course index blade for This course includes --}}
                        <div class="mt-35">
                        ...
                        @if( env('SHOW_COURSE_SOCIAL_LINKS', false) == true)
                            <div class="mt-40 p-10 rounded-sm border row align-items-center favorites-share-box">
                                ...
                                {{-- Hidding the course social links --}}
...
@push('scripts_bottom')
    ...
    <script src="/assets_1/default/vendors/video/video.min.js"></script>
