...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ trans('admin/pages/quizResults.quiz_result_pending_list_page_title') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="/admin/">{{trans('admin/main.dashboard')}}</a>
                </div>
                <div class="breadcrumb-item">{{ trans('admin/main.quizzes') }}</div>
                <div class="breadcrumb-item">{{ trans('admin/pages/quizResults.quiz_result_pending_list_page_title') }}</div>
            </div>
        </div>
                        ...
                        <div class="card-header">
                            @can('admin_quiz_result_export_excel')
                                <div class="text-right">
                                    <a href="/admin/quizzes/pending/excel" class="btn btn-primary">{{ trans('admin/main.export_xls') }}</a>
                                </div>
                            @endcan
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped font-14">
                                    ...
                                    @foreach($quizzesResults as $result)
                                        <tr>
                                            <td>
                                                <span>{{ isset($result->quiz->title) ? ucwords(trim($result->quiz->title)) : "" }}</span>
