...
class AdminAuthenticate
{
    ...
    public function handle($request, Closure $next)
    {
        if (auth()->check() and auth()->user()->isAdmin()) {
            /*this requestedUser key will be created when admin view the user profile
            and will be forgot when admin comes back to its routes */
            if (session()->has('requestedUser')) {

                \Session::forget('requestedUser');
            }
