...
class MeetingController extends Controller
{
    public function reserve(Request $request)
    {
        ...
        //following if block will work only if the oragnizational user is logged in and try to book a meeting
        if (isset($student) && (int)$student > 0 && (auth()->user()->isOrganizationPersonnel()))
        {
            $user = User::where(['id' => (int)$student, 'status' => User::$active])->first();
        } else {
            $user = auth()->user();
        }
            ...
            if (!empty($reserve)) {
                ...
                if (!empty($sale)) {
                    ...
                    $notifyOptions = [
                        ...
                        '[time.date]' => $reserve->day . ' ' . $reserve->time,
                    ];

                    sendNotification('new_appointment', $notifyOptions, $user->id);
                    sendNotification('new_appointment', $notifyOptions, $reserve->meeting->creator_id);
                }
            }
        }
