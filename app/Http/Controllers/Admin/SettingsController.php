...
class SettingsController extends Controller
{
    ...
    public function page($page)
    {
        removeContentLocale();
        if($page == 'sidebar') {
            $settings = SettingSidebar::orderby('id', 'desc')
            ->first();
            $data = [
        ...
        //TODO - check for adding new permission for authorization
        if ($page !== 'organization_financial') {
    ...
    public function store(Request $request, $name)
    {
        ...
        if (!empty($values)) {
            ...
            if ($name == 'organization_financial') {
                cache()->forget(config('cacheKeys.financialTabEnabled'));
            }
    ...
    {
        $settings = new SettingSidebar();
        $settings->new_class = $request->classes;
        $settings->new_quiz = $request->quizess;
        $settings->save();

        return back();
    }

    public function courseCustomization()
    {
        $key = 'drip_feed_on_course_side_bar';
        $status = CustomSetting::where('key' , $key)->first();
        return view('admin.settings.course_general_settings',['status'=>$status]);
    }

    public function courseCustomizationStore (Request $request)
    {
        try {
            ...
            $result = DB::table('custom_settings')->updateOrInsert(
                ['key' => $key],
                ['value' => $value]
            );
            if ($result) {

                return redirect()->route('settings.course.customization');
            } else {
