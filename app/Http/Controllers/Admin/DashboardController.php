...
class DashboardController extends Controller
{
    ...
    /**
     * Method to return details of Sale Stats based on incoming period request.
     * This method has been created to handle incoming ajax result from the Dashboard section
     * @param   $request    Request The incoming Request
     * @param   $period     string  Whether the required period is a day, month or year
     * 
     * This method outputs a json response rather than providing a return value
     */
    public function fetchCurrentSaleStats(Request $request, $period = null) {
        $dateStartAndEnd = $this->getAllDateStartAndEnd();
        //default fallback daterange as null
        $begin  = null;
        $end    = null;
        $heading = null;
        //set beginning and end range based on request
        if (strtolower($period) === 'day') {
            $begin      = $dateStartAndEnd['today']['start'];
            //$begin      = 1625944000; //test value
            $end        = $dateStartAndEnd['today']['end'];
            //$end        = 1626030400; //test value (range set for 24 hours instead of a complete day for testing)
        } else if (strtolower($period) === 'month') {
            $begin      = $dateStartAndEnd['month']['start'];
            $end        = $dateStartAndEnd['month']['end'];
        } else if (strtolower($period) === 'year') {
            $begin      = $dateStartAndEnd['year']['start'];
            $end        = $dateStartAndEnd['year']['end'];
        }
        if ($begin !== null && $end !== null) {
            $heading    = "Sale Stat Details between <em>'". date('d-M-Y H:i:s', $begin). "'</em> and <em>'". date('d-M-Y H:i:s', $end) ."'</em>";
        }
        
        //Append required results relevant parameters to the incoming $request 
        $request->merge([
            'from'      => $begin,
            'to'        => $end,
            'status'    => 'success']);

        $html = (new SaleController)->index($request);
