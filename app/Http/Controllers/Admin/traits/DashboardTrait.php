...
trait DashboardTrait
{
    public function dailySalesTypeStatistics()
    {
        ...
        $courseSales = Sale::whereNull('refund_at')
            ...
            ->whereBetween('created_at', [$beginOfDay, $endOfDay])
            ->whereHas('webinar', function ($query) {
                $query->whereIn('type', [Webinar::$course, Webinar::$textLesson]);
            })->count();
        ...
        $nonSelfPayedSales = Sale::whereNull('refund_at')
            ->where('self_payed', 0)
            ->whereIn('type', [Sale::$webinar, Sale::$meeting])
            ->whereBetween('created_at', [$beginOfDay, $endOfDay])
            ->count();

        $allSales = $allSales - $nonSelfPayedSales;
    ...
    /* Calculating income stats from Sales table instead of Accounting table */
    private function getIncomes($from = null, $to = null)
    {
        $query = Sale::where('self_payed', true);

        $query = fromAndToDateFilter($from, $to, $query, 'created_at', true);
        
        $income = $query->sum('total_amount');
