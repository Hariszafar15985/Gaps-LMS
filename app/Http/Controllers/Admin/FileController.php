...
class FileController extends Controller
{
    ...
    public function destroy(Request $request, $id)
    {
        ...
        if (!empty($file)) {
            $isAttached = TextLessonAttachment::where('file_id',$id)->first();
            if (!$isAttached) {
                ...
                return response()->json([
                    'code' => 422,
                    'message' => 'Detach this file first.'
                ], 422);
            }
        }
    ...
    //following function will use to unzip a file
    ...
    //following function will use to give the manual access to user for drip-feed-file
    public function showManually(Request $request)
    {
        $fileId= $request->file_id; //textLessonId
        $id = $request->userId; //userId

        $fileDetail = File::find($fileId);

        $dripFeedUser = DripFeedFile::where(["user_id" => $id, "file_id" => $fileDetail->id])->first();
        ...
        if ($dripFeedUser) {
            ...
            DripFeedFile::create([
                ...
                "webinar_id" => $fileDetail->webinar_id,
            ]);
        }
