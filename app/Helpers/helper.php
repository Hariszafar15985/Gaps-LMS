...
function getEnrolDate($webinarId, $userId)
{ //this method return the date on which the student is enrolled to the course
    $sale = Sale::where(["webinar_id" => $webinarId, 'buyer_id' => $userId])->first();
    if (isset($sale->created_at)) {
        return date("Y-m-d", $sale->created_at);
    }
}

/**
 * moveAudioFile function:- This function is used to store the audio files into public folder
 *
 * @param Request $request
 * @param object $user
 * @return string
 */
function moveAudioFile(Request $request) {
    $audioFile = $request->file("audio-file");
    $fileUploadPath = "storage/audio_files/";
    $newFileName = time() . "_audio." . $audioFile->getClientOriginalExtension();
    $audioFile->move(public_path('/' . $fileUploadPath), $newFileName);
    return $newFileName;
}

/**
 * This method returns the date on which quiz will be accessible to users
 *
 * @param string $coursePurchaseDate
 * @param integer $showAfterDays
 * @return string
 */
function quizWillShowOn($coursePurchaseDate, int $showAfterDays)
{
    return Carbon\Carbon::create($coursePurchaseDate)->addDays($showAfterDays)->format("d/m/Y");
}

// function getCoursePurchaseDateForQuiz($quizId, $userId)
// { //this method return the date on which the student is enrolled to the course
//     $quiz = Quiz::find($quizId);
//     $sale = Sale::where(["webinar_id" => $quiz->webinar_id, 'buyer_id' => $userId])->first();
//     if (isset($sale->created_at)) {
//         return date("Y-m-d", $sale->created_at);
//     }
// }
...
// block contains the methods which are used in middlewares to check if the webinar content
// is on drip feed and is user has access or not (starts from here)

/**
 * In this method, we are checking followings
 * this method return the date on which the student is enrolled to the course
...
function getCoursePurchaseDate($webinarId, $userId)
{
    $sale = Sale::where(["webinar_id" => $webinarId, 'buyer_id' => $userId])->first();
    if (isset($sale->created_at)) {
        return date("Y-m-d", $sale->created_at);
...
/**
 * This method can user to check that use has manuall access granted of a drip feed quiz,file or text lesson
 * if access granted, method return true else return false
 *
 * @param int $id any['quiz','file','textLesson']
 * @param int $userId
 * @param string $type
 * @return void
 */
function manualAccessGranted(int $id, int $userId, string $type)
{
    if ($type == 'quiz') {
        ...
        $userHasManualAccess = DripFeedFile::where([
            "file_id" => $id,
            "user_id" => $userId,
        ])->first();

    } elseif ($type == 'text_lesson'){
        $userHasManualAccess = DripFeedUser::where([
            "text_lesson_id" => $id,
            "user_id" => $userId,
        ])->first();
    }
    return !empty($userHasManualAccess);
}

/**
 * In this method, we are checking followings
 * 1. Quiz is on drip feed or not
 * 2. if quiz is on drip feed, then user has manuall access or not
 *
 * @param integer $quizId
 * @param integer $userId
 * @return boolean
 */
function userCanAttemptQuiz(int $quizId, int $userId)
{
    ...
    $quiz = Quiz::find($quizId);
    if (!empty($quiz)) {
        if (!empty($quiz->drip_feed) && (int)$quiz->drip_feed == 1) {
            $coursePurchaseDate = getCoursePurchaseDate($quiz->webinar_id, $userId);
            $date1 = Carbon\Carbon::create($coursePurchaseDate)->addDays($quiz->show_after_days)->format("Y-m-d");
            $date2 = Carbon\Carbon::create(Carbon\Carbon::today())->format("Y-m-d");

            $dripFeedDate = Carbon\Carbon::createFromFormat('Y-m-d', $date1);
            $currentDate = Carbon\Carbon::createFromFormat('Y-m-d', $date2);

            $result = $currentDate->gte($dripFeedDate);

            if ($result === false) {
                return manualAccessGranted($quiz->id, $userId, 'quiz');
            } else {
                return true;
            }
        } else {
...
/**
 * In this method, we are checking followings
 * 1. file is on drip feed or not
 * 2. if file is on drip feed, then user has manuall access or not
 * @param int $fileId
 * @param int $userId
 * @return bool true or false
 */

function userCanAttemptFile(int $fileId, int $userId)
{
    $user = User::find($userId);
    ...
    if (!empty($file)) {
        if (!empty($file->drip_feed) && (int)$file->drip_feed == 1) {
            $coursePurchaseDate = getCoursePurchaseDate($file->webinar_id, $userId);
            $date1 = Carbon\Carbon::create($coursePurchaseDate)->addDays($file->show_after_days)->format("Y-m-d");
            $date2 = Carbon\Carbon::create(Carbon\Carbon::today())->format("Y-m-d");
            ...
            if ($result === false) {
                return manualAccessGranted($file->id, $userId, 'file');
            } else {
                return true;
            }
        } else {
...
/**
 * In this method, we are checking followings
 * 1. TextLesson is on drip feed or not
 * 2. if TextLesson is on drip feed, then user has manuall access or not
 * @param int $fileId
 * @param int $userId
 * @return bool true or false
 */
function userHasDripFeedAccess($textLessonId, $userId)
{
    $user = User::find($userId);
    if ($user->isAdmin() || !$user->isUser()) {
        return true;
    }
    $textLesson = TextLesson::find($textLessonId);
    if (!empty($textLesson)) {
        if (!empty($textLesson->drip_feed) && (int)$textLesson->drip_feed == 1) {
            $coursePurchaseDate = getCoursePurchaseDate($textLesson->id, $userId);
            $date1 = Carbon\Carbon::create($coursePurchaseDate)->addDays($textLesson->show_after_days)->format("Y-m-d");
            $date2 = Carbon\Carbon::create(Carbon\Carbon::today())->format("Y-m-d");
            ...
            if ($result === false) {
                return manualAccessGranted($textLesson->id, $userId, 'text_lesson');
            } else {
                return true;
            }
        } else {
...
// block contains the methods which are used in middlewares to check if the webinar content
// is on drip feed and is user has access or not (ends here)


//course will show on
function courseWillShowOn($coursePurchaseDate, $showAfterDays)
{
    return Carbon\Carbon::create($coursePurchaseDate)->addDays($showAfterDays)->format("d/m/Y");
}

//check the lesson is allowed to show manually
function showManually($textLessonId, $userId)
{
    $dripFeedUser = App\DripFeedUser::where(["user_id" => $userId, "text_lesson_id" => $textLessonId])->first();
    //($dripFeedUser) ? $manuallyShow = true : $manuallyShow = false;
    return !empty($dripFeedUser);
}

//this method return previous key of array
...
//this method check that the course is drip feed or not
function isCourseOnDripFeed($textLessonId, $userId)
{
    if ($textLessonId) {
        $textLesson = TextLesson::find($textLessonId);
        if (!empty($textLesson->drip_feed) && (int) $textLesson->drip_feed == 1) {
            $coursePurchaseDate = getCoursePurchaseDate($textLesson->webinar_id, $userId);
            $date1 = Carbon\Carbon::create($coursePurchaseDate)->addDays($textLesson->show_after_days)->format("Y-m-d");
            $date2 = Carbon\Carbon::create(Carbon\Carbon::today())->format("Y-m-d");
...
function showToUsers($textLessonId)
{ //this method return listing of that users which have manually access to view the lesson
    $textLesson = TextLesson::find($textLessonId);
    if ($textLesson->show_to_users == "") {
        $usersArray = array(0);
    } else {
        $usersArray = json_decode($textLesson->show_to_users);
    }
    return $usersArray;
}
...
/**
 * Return the system's (global) Organization Financial Settings as an Associative Array
 *
 * @return array
 */
function getOrganizationFinancialSettings()
{
    $settings = \Illuminate\Support\Facades\DB::table('settings')
        ->join('setting_translations', 'settings.id', '=', 'setting_translations.setting_id')
        ->select('setting_translations.value')
        ->where('page', App\Models\Setting::$organizationFinancial)
        ->first();

    return !empty($settings->value) ? json_decode($settings->value, true) : [];
}

/**
 * Return value from cache whether the financial tab link in organization users' sidebar
 * has been enabled in system settings
 *
 * @return boolean
 */
function isFinancialTabEnabled()
{
    //Remember setting in cache for 1 day
    return Illuminate\Support\Facades\Cache::remember(config('cacheKeys.financialTabEnabled'), 24 * 60 * 60, function () {
        $settings = getOrganizationFinancialSettings();
        if (!empty($settings) && isset($settings['financeTab'])) {
            return (int)$settings['financeTab'] === 1;
        }
        return false;
    });
}
...
function sendNotification($template, $options, $user_id = null, $group_id = null, $sender = 'system', $type = 'single')
{
    ...
    if (!empty($notificationTemplate)) {
        ...
        $check = \App\Models\Notification::where(
                    ...
                    if (env('APP_ENV') == 'production') {
                        ...
                        if (!empty($user) and !empty($user->email)) {
                            try {
                                ...
                                $message = "[ER:MAIL_NOTIF_BULK_LOOP] Failed to send Notification email - ";
                ...
                if (env('APP_ENV') == 'production') {
                    ...
                    if (!empty($user) and !empty($user->email)) {
                        try {
                            ...
                            $message = "[ER:MAIL_NOTIF_SINGLE_USR] Failed to send Notification email - ";
                            $message .= $exception->getMessage();
                            \Log::error($message);
                        }
                    }
...
function fromAndToDateFilter($from, $to, $query, $column = 'created_at', $strToTime = true)
{
    if (!empty($from) and !empty($to)) {
        if (is_int($from) && $from > 0) { //in case already int timestamp is received
            $from = date('Y-m-d H:i:s', $from);
        }
        $from = $strToTime ? strtotime($from) : $from;
        if (is_int($to) && $to > 0) { //in case already int timestamp is received
...
/**
 * Function to get IP address of User/Client
 *
 * @return string
 */
...
/**
 * Function to log audit trail at the time of user logout
 * This method is currently only setup to logs this activity for students
 *
 * @return void
 */
function logLogoutActivity()
{
    ...
    if (isset($user->id) && $user->id > 0) {
        ...
        $ip = getClientIp();
        $auditMessage = "User logged out of account";
        $audit = new AuditTrail();
        $audit->user_id = $user->id;
        $audit->organ_id = $user->organ_id;
        $audit->role_name = $user->role_name;
        $audit->audit_type = AuditTrail::auditType['logout'];
...
function uploadFile($fileRequest, $fileName, $fileUploadPath)
{
    $newFileNames = [];
    $files = $fileRequest->file($fileName);
    if($fileRequest->hasFile($fileName)) {
        if (is_array($files)) {
            foreach($files as $file){
                $newFileNames[] = moveFile($file, $fileUploadPath);
            }
            return $newFileNames;
        } else {
            return moveFile($files, $fileUploadPath);
        }
    }
}

function moveFile($file, $fileUploadPath) {
    $image = $file;
    $newFileName = mt_rand(100, date("Ymd")).md5_file($image->getRealPath()) . '.' . $image->getClientOriginalExtension();
    //check if directory exists
    if ( !file_exists( public_path('/'.$fileUploadPath) ) ) {
        mkdir(public_path('/'.$fileUploadPath),0777,true);
    }
    $image->move(public_path('/'.$fileUploadPath), $newFileName);
    return $newFileName;
}

function getFileType($fileName) {
    $fileNameExplod = explode("." , $fileName);
    if(is_array($fileNameExplod)) {
        return $fileNameExplod[sizeof($fileNameExplod) - 1];
    } else {
        return $fileName;
    }
}
